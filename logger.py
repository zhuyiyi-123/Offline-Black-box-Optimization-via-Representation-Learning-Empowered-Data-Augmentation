        ...
        with self.writer.as_default():
            ...
            if tf.equal(tf.size(value), 1):
                ...
                tf.summary.scalar(key + '/100th',
                                  ...
                                  step=step)
                print("100th:", tfp.stats.percentile(value, 100.0))
                ...
                tf.summary.scalar(key + '/max',
                                  ...
                                  step=step)
                # print(key + '/max', tf.math.reduce_max(value), step)
                tf.summary.scalar(key + '/mean',
                                  tf.math.reduce_mean(value),
                                  step=step)
                # print(key + '/mean', tf.math.reduce_mean(value), step)
                tf.summary.scalar(key + '/min',
                                  tf.math.reduce_min(value),
                                  step=step)
                # print(key + '/min', tf.math.reduce_min(value), step)
                tf.summary.scalar(key + '/std',
                                  tf.math.reduce_std(value),
                                  step=step)
                # print(key + '/std', tf.math.reduce_std(value), step)
